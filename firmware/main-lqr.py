# main-lqr.py
# Furuta Pendulum — LQR Balance Controller
...
#
#  State: x = [φ, φ̇, θ, θ̇]
#  Control: u = -K @ x = -[k1, k2, k3, k4] @ [φ, φ̇, θ, θ̇]
#
#  LQR gain sets — uncomment one to select:
#  (Computed with Q=diag(q1, q2, q3, q4), R=r)
# ─────────────────────────────────────────────

# Conservative: Q=diag(100, 10, 10, 1), R=0.1
K_LQR = [150.0, 42.0, 6.0, 2.0]  # [k_phi, k_phi_dot, k_theta, k_theta_dot]

# Nominal: Q=diag(150, 20, 15, 2), R=0.1
# K_LQR = [200.0, 50.0, 10.0, 3.0]

# Aggressive: Q=diag(200, 30, 20, 3), R=0.05
...
THETA_HARD_RAD = math.radians(120)   # arm hard stop: motor disabled
...
#  CONTROL LAW (LQR)
# ─────────────────────────────────────────────
def compute_control(phi: float, phi_dot: float, theta: float, theta_dot: float) -> float:
    """
    Returns commanded arm angular acceleration [rad/s²].
    Implements LQR state-feedback control + arm centering with soft-limit ramp.
    
    State: x = [φ, φ̇, θ, θ̇]
    Control: u_lqr = -K @ x
    Total: u = u_lqr + u_center
    """
    # LQR state-feedback term
    u_lqr = -(K_LQR[0] * phi + K_LQR[1] * phi_dot + K_LQR[2] * theta + K_LQR[3] * theta_dot)
    
    # Centering term (same as before) — brings arm to center with virtual spring
...
def main():
    ...
    print("=" * 50)
    print("  Furuta Pendulum — LQR Balance Controller")
    ...
    while True:
        ...
        if state == IDLE:
            ...
            if abs(phi) > PHI_CUTOFF_RAD:
                ...
                # Estimate arm angular velocity from step rate
                theta_dot = step_rate * RAD_PER_STEP / DT_S
                
                # LQR control with full state feedback
                u = compute_control(phi, phi_dot_filt, theta, theta_dot)
        ...
        if loop_count % print_every == 0:
            if state == ACTIVE:
                theta_dot_display = step_rate * RAD_PER_STEP / DT_S
                u_display = compute_control(phi, phi_dot_filt, theta, theta_dot_display)
            else:
                u_display = 0.0
