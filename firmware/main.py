...
# ─────────────────────────────────────────────
ZERO_RAW = 2048    # raw AS5600 count when pendulum is exactly upright
...
K_CENTER = 1.5    # arm centering gain [rad/s² per rad of arm displacement]
...
ALPHA_EMA      = 0.20           # velocity EMA smoothing (tune from test_velocity.py)
N_DIFF         = 3              # finite difference window (samples)

ALPHA_MAX      = 600.0          # max arm angular accel command [rad/s²]
MAX_STEP_RATE  = 800.0          # max step rate [steps/s] — safe torque zone
MAX_STEP_ACCEL = 4000.0         # step rate ramp limit [steps/s²]

THETA_SOFT_RAD = math.radians(50)   # arm soft limit: centering ramps up
THETA_HARD_RAD = math.radians(60)   # arm hard stop: motor disabled
PHI_CUTOFF_RAD = math.radians(15)   # pendulum fall threshold → RECOVERING
PHI_ACTIVE_RAD = math.radians(5)    # must be within this to activate/recover
...
# ─────────────────────────────────────────────
def main():
    global state, phi_history, phi_dot_filt, idle_timer_ms
    ...
    while True:
        ...
        if state == IDLE:
            ...
            if abs(phi) > PHI_CUTOFF_RAD:
                state = RECOVERING
                motor_disable()
                print(f">>> RECOVERING — φ = {math.degrees(phi):.1f}°  (> 15°)")
            elif abs(theta) > THETA_HARD_RAD:
                state = LIMIT_HIT
                motor_disable()
                print(f">>> LIMIT_HIT — arm at {math.degrees(theta):.1f}°")
            else:
                # Normal control
            ...
            motor_disable()
            if abs(theta) < THETA_SOFT_RAD and abs(phi) < PHI_ACTIVE_RAD:
                idle_timer_ms += DT_S * 1000
                if idle_timer_ms >= 500:
                    state        = ACTIVE
                    phi_dot_filt = 0.0
                    motor_enable()
                    print(">>> ACTIVE — arm limit cleared")
            else:
                idle_timer_ms = 0
